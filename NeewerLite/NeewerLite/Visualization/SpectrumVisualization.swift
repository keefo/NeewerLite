...
class SpectrumVisualization: MTKView, MTKViewDelegate {
    ...
    /// Decaying peak tracker: rises instantly, decays slowly for smooth normalization.
    private var trackingPeak: Float = 0
    private let peakDecay: Float = 0.995  // ~3 seconds to halve at 60fps
    ...
    private func applyFrequencyData(_ frequencyData: [Float]) {
        ...
        let framePeak = frequencyData.max() ?? 0
        if framePeak.isFinite && framePeak > trackingPeak {
            trackingPeak = framePeak
        } else {
            trackingPeak *= peakDecay
        }

        // Leave ~8% padding at the top so bars don't touch the ceiling.
        let maxHeight = Float(viewHeight) * 0.92
        let processed = normalizedFrequencyData(frequencyData, referencePeak: trackingPeak)

        let alpha = smoothingAlpha
        let oneMinusAlpha = 1.0 - alpha

        for idx in 0..<num {
            let target = processed[idx] * maxHeight
            let smoothed = smoothedHeights[idx] * oneMinusAlpha + target * alpha
            smoothedHeights[idx] = smoothed.isFinite ? smoothed : 0
        }
    }
    ...
    func draw(in view: MTKView) {
        ...
        // Always render current smoothedHeights, even when no new data arrived.
        guard !smoothedHeights.isEmpty else { return }
