...
- [Neewer Home - NS02 Neon Rope Light — BLE Protocol Reference](#neewer-home---ns02-neon-rope-light--ble-protocol-reference)
  ...
  - [Commands](#commands)
    ...
    - [HSI / Color Mode](#hsi--color-mode)
      - [Mode 0x01 — Whole Light (Solid Color)](#mode-0x01--whole-light-solid-color)
      - [Mode 0x02 — Per-Segment (Multiple Colors)](#mode-0x02--per-segment-multiple-colors)
    ...
    - [Music Reactive Mode](#music-reactive-mode)
      ...
      - [Music Mode IDs](#music-mode-ids)
      - [Default Auto Colors (colorMode = 0x01)](#default-auto-colors-colormode--0x01)
      - [Decoded BLE Traces](#decoded-ble-traces)
      - [Device Support](#device-support)
    - [Scene Effect](#scene-effect)
      - [Payload Structure](#payload-structure-1)
      - [Sub-block Structure (effectMode 13 — "Progressive")](#sub-block-structure-effectmode-13--progressive)
      - [Sub-block Structure (effectMode 1 — "Breath")](#sub-block-structure-effectmode-1--breath)
      - [Effect Modes (effectMode values)](#effect-modes-effectmode-values)
      - [Effect Sub-block Parameters (Advanced)](#effect-sub-block-parameters-advanced)
      - [Decoded BLE Traces](#decoded-ble-traces-1)
      - [Scene Catalog](#scene-catalog)
      - [Scene Command Implementation](#scene-command-implementation)
      - [Pre-built Command Shortcut](#pre-built-command-shortcut)
...
Long Size Packet. All color commands share dataId `0x0D` and use LongSizePacket (2-byte big-endian size). The `lightColorType` byte (data[2]) determines the mode:
...
These additional parameters may appear in the sub-block depending on effectMode:
...
> The exact sub-block byte layout per effectMode requires BLE capture for each mode. Not all parameters apply to every mode.
...
For built-in scenes, only brightness is adjusted at send time — all other parameters (speed, colors, animation mode) are fixed in the pre-built packet.
