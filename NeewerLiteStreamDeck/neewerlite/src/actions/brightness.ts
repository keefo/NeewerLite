import streamDeck, { action, DialAction, DialRotateEvent, DialUpEvent, DidReceiveSettingsEvent, SingletonAction, WillAppearEvent, type JsonValue, type KeyDownEvent, type SendToPluginEvent } from "@elgato/streamdeck";
						...
						action.getSettings().then((settings) => {
							for (const light of settings.selectedLights)
                            {
                                for (const remote_light of lights)
                                {
                                    ...
                                    {
                                        if (remote_light.brightness != settings.brightness || (remote_light.state == 1) != settings.light_state)
                                        {
